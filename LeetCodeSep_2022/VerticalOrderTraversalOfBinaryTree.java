...
public class VerticalOrderTraversalOfBinaryTree {
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        Map<Integer, List<Integer>> lineVsVal = new TreeMap<>();
        ...
        while (!qu.isEmpty()) {
            ...
            for (Map.Entry<Integer, Queue<Integer>> entry : sMap.entrySet()) {
                ...
                lineVsVal.putIfAbsent(line, new ArrayList<>());
                Queue<Integer> pq = entry.getValue();
                while (!pq.isEmpty()) {
                    lineVsVal.get(line).add(pq.poll());
                }
            }
        }

        for (List<Integer> list : lineVsVal.values()) {
            ans.add(list);
        }
        return ans;
    }
